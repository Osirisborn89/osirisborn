param()
# Simple JSON helpers for data/learn/<lang>.json manifests

function Get-LangManifest {
  param([Parameter(Mandatory)][string]\)
  \ = Join-Path 'MythicCore\\www\\data\\learn' (\.ToLower() + '.json')
  if (!(Test-Path \)) { throw "Manifest not found: \" }
  (Get-Content -Raw -Path \ -Encoding UTF8) | ConvertFrom-Json
}

function Save-LangManifest {
  param([Parameter(Mandatory)][psobject]\)
  if (-not \.lang) { throw "Manifest.lang missing" }
  \ = Join-Path 'MythicCore\\www\\data\\learn' (\.lang.ToLower() + '.json')
  (\ | ConvertTo-Json -Depth 20) | Set-Content -Path \ -Encoding UTF8
  Write-Host "Saved -> \" -ForegroundColor Green
}

function New-LangManifest {
  param([Parameter(Mandatory)][string]\, [string]\)
  \ = \.ToLower()
  \ = [pscustomobject]@{ lang = \; title = (\ ? \ : \); modules = @(
      [pscustomobject]@{ id='intro'; title='Basics'; lessons=@(
        [pscustomobject]@{ id = "\-intro-01"; title="Introduction";    mins=5;  html="<p>Coming soon.</p>" },
        [pscustomobject]@{ id = "\-intro-02"; title="Setup & Tooling"; mins=10; html="<p>Coming soon.</p>" },
        [pscustomobject]@{ id = "\-intro-03"; title="Syntax Primer";   mins=15; html="<p>Coming soon.</p>" }
      ) }
  ) }
  Save-LangManifest -Manifest \
}

function Add-LangModule {
  param([Parameter(Mandatory)][string]\,[Parameter(Mandatory)][string]\,[Parameter(Mandatory)][string]\)
  \ = Get-LangManifest -Lang \
  if ((\.modules | Where-Object id -eq \)) { throw "Module exists: \" }
  \.modules += [pscustomobject]@{ id=\; title=\; lessons=@() }
  Save-LangManifest -Manifest \
}

function Remove-LangModule {
  param([Parameter(Mandatory)][string]\,[Parameter(Mandatory)][string]\)
  \ = Get-LangManifest -Lang \
  \.modules = @(\.modules | Where-Object { \.id -ne \ })
  Save-LangManifest -Manifest \
}

function Add-LangLesson {
  param(
...
.tab.active{ box-shadow:0 0 0 2px rgba(139,92,246,.45) }
  ...
  <style>
    /* wipe-learn-python */
    body.route-wipe-python #python-blank { display:block; }
    #python-blank { display:none; min-height:50vh; }
    /* Hide everything except NAV, our blank container, and footer */
    body.route-wipe-python > *:not(nav):not(#python-blank):not(.footer) { display:none !important; }
    /* Ensure background/theme remain consistent */
    body.route-wipe-python { background: inherit !important; color: inherit !important; }
  </style>  <script>
    // wipe-learn-python route guard
    (function(){
      function ensureBlank(){
        try {
          var onPy = (location.hash || '').toLowerCase().indexOf('#/learn/python') === 0;
          document.body.classList.toggle('route-wipe-python', onPy);
          if (onPy && !document.getElementById('python-blank')) {
            var blk = document.createElement('div');
            blk.id = 'python-blank';
            // Intentionally empty canvas; keep top nav + theme only.
            document.body.insertBefore(blk, document.querySelector('.footer') || null);
          }
        } catch(e){}
      }
      window.addEventListener('hashchange', ensureBlank);
      document.addEventListener('DOMContentLoaded', ensureBlank);
      window.addEventListener('portal:ready', ensureBlank);
      ensureBlank();
    })();
  </script></head>
<body>
...
(function(){
 ...
 = "<p>Coming soon.</p>"
  ...
  \ = Get-LangManifest -Lang \
  \ = \.modules | Where-Object id -eq \
  if (-not \) { throw "Module not found: \" }
  if ((\.lessons | Where-Object id -eq \)) { throw "Lesson exists: \" }
  \.lessons += [pscustomobject]@{ id=\; title=\; mins=\; html=\<!doctype html>
<html lang="en">
<head>
  <meta charset="utf-8"/>
  <meta name="viewport" content="width=device-width,initial-scale=1"/>
...
(function(){
  ...
  \ = \
  foreach (\ in \.modules) {
    \ = \.lessons | Where-Object id -eq \
    if (\) { break }
  }
  if (-not \) { throw "Lesson not found: \" }
  if (\System.Management.Automation.PSBoundParametersDictionary.ContainsKey('Title')) { \.title = \ }
  if (\System.Management.Automation.PSBoundParametersDictionary.ContainsKey('Mins'))  { \.mins  = \ }
  if (\System.Management.Automation.PSBoundParametersDictionary.ContainsKey('Html'))  { \.html  = \<!doctype html>
<html lang="en">
<head>
  <meta charset="utf-8"/>
  <meta name="viewport" content="width=device-width,initial-scale=1"/>
...
function Remove-LangLesson {
  param([Parameter(Mandatory)][string]\,[Parameter(Mandatory)][string]\)
  \ = Get-LangManifest -Lang \
  foreach (\ in \.modules) {
    \.lessons = @(\.lessons | Where-Object { \.id -ne \ })
  }
  Save-LangManifest -Manifest \
}

Export-ModuleMember -Function *-Lang*
