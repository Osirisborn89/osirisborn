/* Black Pyramid — LMS Chrome v1 (hardened)
 * Mounts inside #host if present; lesson-only; resilient to SPA wipes.
 */
(function () {
  "use strict";
  ...
  function ensureStyles(){
    ...
    const css = `
    #host .bp-lms, .bp-lms { font-family: system-ui, Segoe UI, Roboto, Helvetica, Arial, sans-serif; line-height:1.45; color:#e6e6e6; }
    .bp-debug-banner { position:fixed; bottom:8px; right:8px; z-index:99999; font:12px/1.4 system-ui; padding:6px 8px; border-radius:6px; background:#111a; color:#e6e6e6; border:1px solid #2b3845; opacity:.95 }
    .bp-wrap { display:grid; grid-template-columns: 280px 1fr; gap:16px; margin-top:8px; }
  ...
  function showDebug(txt){
    let d=qs("#bp-debug");
    if(!d){ d=document.createElement("div"); d.id="bp-debug"; d.className="bp-debug-banner"; (document.body||document.documentElement).appendChild(d); }
    d.textContent=txt;
  }
  ...
  function parseRoute(){
    ...
    const route = { ok: true, trackId: parts[1], moduleId: parts[2], lessonId: parts[3] };
    return route;
  }
  ...
  function computeTrackProgress(track){
    ...
    const pct= total? Math.round(done*100/total):0; return { doneCount:done, totalCount:total, pct };
  }
  ...
  function mountHost(){
    // Prefer #host if present (portal guidance), else body
    const root = qs("#host") || document.body || document.documentElement;
    let host = qs("#bp-lms", root);
    if(!host){ host = document.createElement("div"); host.id="bp-lms"; host.className="bp-lms"; root.appendChild(host); }
    return host;
  }
  ...
  async function render(){
    ...
    let track;
    try{
      track=await getTrack(r.trackId);
    }catch(e){
      LOG("fetch failed", e);
      teardown();
      showDebug(`LMS DEBUG — ERROR loading ${r.trackId}.json`);
      return;
    }
  ...
  // Poll hash in case SPA mutates silently
  ...
  // Observe SPA wipes (remove of #bp-lms)
  const mo = new MutationObserver(()=>{ if(!qs("#bp-lms")){ LOG("host removed → re-mount"); scheduleRender(); } });
