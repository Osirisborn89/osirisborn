...
BeforeAll {
  ...
  pwsh -NoProfile -File .\tools\Reset-Server.ps1 | Out-Null
...
(function(){
 ...
 = Get-Content -Raw -Path .\MythicCore\www\portal.standalone.html -Encoding UTF8
    \<!doctype html>
<html lang="en">
<head>
  <meta charset="utf-8"/>
...
(function(){
 ...
 | Should -Match 'langs\\.lessons\\.css'
    \<!doctype html>
<html lang="en">
<head>
  <meta charset="utf-8"/>
...
(function(){
 ...
 | Should -Match 'js/langs\\.loader\\.js'
  }

  It 'Python manifest parses and has modules/lessons (if present)' -TestCases @{ Lang='python' } {
    param(\)
    \ = Join-Path .\MythicCore\www\data\learn ("\.json")
    if (Test-Path \) {
      \ = (Get-Content -Raw -Path \ -Encoding UTF8) | ConvertFrom-Json
      \.modules.Count | Should -BeGreaterThan 0
      (\.modules[0].lessons.Count) | Should -BeGreaterThan 0
    } else {
      Set-ItResult -Skipped -Because "Manifest missing — placeholder route expected"
    }
  }
